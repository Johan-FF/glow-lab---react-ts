...
export const FavoritesSidebar: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50" role="presentation">
      <button
        type="button"
        className="absolute inset-0 bg-black/40 animate-fade-in border-0 p-0 cursor-default"
      ...
      <aside
        ...
        className="absolute right-0 top-0 h-full w-full max-w-md bg-background shadow-strong border-l z-10 animate-slide-in-right"
      >
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between p-6 border-b">
            <div className="flex items-center gap-2">
              <Heart className="h-5 w-5 text-primary fill-primary" aria-hidden="true" />
              <h2 id="favorites-sidebar-title" className="text-lg font-semibold">
                ...
                {favorites.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      ...
                      >
                        <ShoppingBag className="h-3 w-3 mr-2" aria-hidden="true" />
                        Agregar a la bolsa
                      </Button>
                    </div>
                  </li>
          ...
          {favorites.length > 0 && (
            <div className="p-6 border-t">
              <Button variant="outline" className="w-full" onClick={() => setIsFavoritesOpen(false)}>
                Seguir explorando
              </Button>
            </div>
          )}
